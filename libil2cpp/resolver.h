...
namespace Il2Cpp
{
    namespace Resolver
    {
        ...
        template <typename T>
        constexpr T GetInternalCall(std::string_view _Name) { return reinterpret_cast<T>(ResolveInternalCall(_Name)); }
        template <typename T>
        constexpr T GetExport(std::string_view _ExportName) { static HMODULE _Module = GetModuleHandleA("GameAssembly.dll"); return reinterpret_cast<T>(GetProcAddress(_Module, _ExportName.data())); }
    }
}